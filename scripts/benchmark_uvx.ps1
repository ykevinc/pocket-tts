...
$finalOutput = "benchmark_results_uvx_$timestamp.md"
...
foreach ($i in 0..($texts.Length - 1)) {
    ...
    $pyCmd = "uvx pocket-tts generate --text ""$text"" --output-path bench_py.wav"
...
" # Benchmark Results (uvx) - $timestamp`n" | Out-File -FilePath $finalOutput -Encoding utf8
